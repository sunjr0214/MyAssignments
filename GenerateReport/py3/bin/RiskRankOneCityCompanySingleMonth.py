...
if __name__ == '__main__':
    ...
    # excel 表头配置
    ExcelTiTleCode1 = eval(conf.getConfig('RiskRankOneCitySingleMonth', 'ExcelTiTleCode1'))

    print('正在生成 %s 区域企业风险排名报表，请稍后...' % (cityName))
    ...
    # 维表：城市、运管所、企业
    companyDimSql = dao.getCompanyDimSql(endMonthStr, cityId)
    ...
    companyDimDF = dao.generateDF(companyDimSql, mysqlEngine)
    ...
    riskRankBigTab = tp.generateRiskRank(vehLinkedDF, netCheckDF, groupColName, companyDimDF, 1)  # 1表示统计夜间禁行
    # print(riskRankBigTab1)

    # 存储数据
    TemplateExcPrefix = 'Jiangsu_RiskRank_OneCity_Company'
    saveExcPrefix = 'Jiangsu_RiskRank_%s_Company_%s-%s' % (cityName, startDay, endDay)
    # savePath格式: Jiangsu_RiskRank_{cityName}_Company_SingleMonth_{统计数据时间区域}_{报表的生成时间}.xlsx
    savePath = s2e.saveOneCitySingleMonthData(TemplateExcPrefix, saveExcPrefix, riskRankBigTab, ExcelTiTleCode1,
                                              firstCellLoc)
    print('%s 区域企业风险排名报表生成完毕，保存路径：%s' % (cityName, savePath))
